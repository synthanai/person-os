...
> **நூல்**: **WHY** , Isolation breeds fragile logic. | **WHAT TYPE** , EXTERNALIZATION (multi-agent spar) | **HOW** , Adopt Personas → Spar → Synthesize → Persist

# /spar , Stress-Test Decisions

> *"Do not let a decision sit in silence. Put it in the arena."*

**SPAR** is the meta-stress-testing verb. Where all other verbs extract and synthesize, SPAR challenges. It spins up temporary adversarial personas rooted in the user's established CORE and Strategy Vector, then forces them into dialectic conflict to expose blind spots, risk vectors, and unconsidered alternatives before a major commitment is executed.

**Pipeline position**: META VERB (run before any major commitment in the GROWTH phase)
**Reads**: `profile/CORE.md`, `strategise/*.md` (REQUIRED — the spar must be grounded in the user's actual context, not generic logic)
**Writes to**: `spar/[dilemma-slug].md`
...
- **Success-Signal**: The spar produces a concrete Verdict that either confirms the original decision with noted risks or proposes a modified path. The Verdict must be falsifiable (the user can evaluate it in 90 days).
- **Synthesis**: Records the full rationale behind major decisions (`spar/[dilemma].md`) for auditability and retrospective LOON generation.
...
- **Source**: `profile/CORE.md` (Extract the Calling and Reason — these are the values the spar must honor or challenge)
...
Execute the spar using the **SPAR** protocol. All spar steps run openly in the chat so the user can observe the cognitive stress-testing.
...
- **90-day check-in**: The specific signal that, if observed, should trigger a re-spar.

---

## Step 4: Mandatory Persist

**File:** `spar/[dilemma-slug].md`

**Header format:**
```
# [Role/Domain]: Decision Spar

**Timestamp**: YYYY-MM-DD
**Framework**: SPAR (S.P.A.R.)
